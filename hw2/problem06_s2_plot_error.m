...
# The network is the same as Lecture 6, Slide 11. Have a look there!

# Init w1, b1, w2, b2
W1(:, 1) = xMin + rand(1, 2)*difference; # For the first layer (logsig)
B1(:, 1) = xMin + rand(1, 2)*difference; # The new values of the weights, biases will be added to each column

W2(1, :) = (xMin+rand(1, 2)*difference)'; # For the second layer (purelin)
...
while (1)
  for p = -2:0.01:2 # For each input
    ...
    error = 1 + sin(p * (pi/2)) - A2(i); # Cost function = Actual - Approximate
